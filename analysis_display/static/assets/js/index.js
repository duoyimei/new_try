...
function initMapData(mapJson) {
    var mapData = [];
    var returnData = ['北京', '海淀区', '上海', '黄埔区'];
    // var returnAllData = []
    var value_list = [];
    for (var i = 0; i < mapJson.features.length; i++) {
        if(isInArray(returnData, mapJson.features[i].properties.name)){
            mapData.push({
...
function registerAndsetOption(myChart, id, name, mapJson, flag) {
    ...
    myChart.setOption({
        tooltip: {
            trigger: 'item',
            formatter: '{b}<br/>{c}(值) <br/>{d}(句子)(p / km2)'
        },
        visualMap: {
            min: 0,
            max: 1,
            text:['involve','uninvolved'],
            realtime: false,
            calculable: true,
            inRange: {
        ...
        series: [{
            ...
            itemStyle: {
                normal: {
                    label:{show:false},
                    areaColor: 'rgba(255, 228, 225,0.5)',
                    borderColor: '#51c1c1',
                    borderWidth: 1,
                },
